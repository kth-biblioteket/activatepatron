...
if(!empty($_REQUEST['auth'])) {
	...
	?>
	<form id="loginuserform" action="activatepatron_aj.php?auth=<?php echo $_REQUEST['auth'] ?>" method="post">
		<div>
			Username
		</div>
		<div>
	...
	if(!empty($_POST['username']) && !empty($_POST['password'])) {
		...
		if($_REQUEST['auth'] == 'alma') {
			if ($loginresponse->result == "Success") {
				...
				if($_REQUEST['auth'] == 'ldap') {
					header("location: /activatepatron/activatepatron_aj.php?auth=ldap" );
				}
